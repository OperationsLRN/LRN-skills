...
| Item | Value | Notes |
|---|---|---|
| Bot/agents | skillmaster (G:\AI\skills) | Compare LLM models against a given skill or task. Given a skill file and a set of test inputs, run each model, capture outputs and token/latency metrics, and produce a ranked comparison report. Runs locally and on AWS AgentCore. Informs which model to specify in each SKILL.md. |
...
| Extract deterministic helpers before calling AI for llm-bench. |
